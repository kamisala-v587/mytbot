...
```text
你正在维护 /vla/workspace/my_tbot。先阅读 .rag/README.md；涉及效果判断时优先检查 .rag/07-experiment-evidence.md；涉及 dataloader、视频解码、训练吞吐、worker/rank 拖尾或 backend/cache 时必须先读 .rag/08-bpva-data-training-benchmark.md，并严格区分历史实验运行条件与当前配置快照。区分【用户实验】与【代码事实】；再按索引阅读相关专题，最后回查当前源码与实际配置。回答和修改时区分【代码事实】【公开资料】【推断】【待验证】，代码事实给精确路径+行号，公开资料给原始 URL；不确定就标待验证。
