...
class TBotSA1Config(PreTrainedConfig):
    _canonical_type: ClassVar[str] = TBOT_SA1  # 策略类型注册名，固定为 TBot_SA1

    # ---- 模型骨干 ----
    qwen3_vl_variant: str = "qwen3_vl_2b"  # und expert 规模，如 qwen3_vl_28l（层数写在后缀）
    action_expert_variant: str = "qwen3_600m"  # gen/act expert 规模，如 qwen3_28l
    qwen3_vl_pretrained_path: str = "Qwen/Qwen3-VL-2B-Instruct"  # Qwen3-VL 预训练权重路径或 HF repo
    dtype: str = "bfloat16"  # 主计算精度：bfloat16 或 float32

    # ---- Action Chunking（一次预测多步动作）----
    n_obs_steps: int = 1  # 观测历史步数（TBot 默认单帧）
    chunk_size: int = 50  # 模型一次预测的 action 序列长度（训练与推理上限）
    n_action_steps: int = 50  # 推理时实际执行的步数，须 ≤ chunk_size

    # ---- 状态/动作维度（padding 到统一宽度）----
    max_state_dim: int = 32  # state 向量填充后的最大维度
    max_action_dim: int = 32  # action 向量填充后的最大维度
    mask_action_dim_padding_loss: bool = False  # 是否在 loss 中忽略 padding 的 action 维度
    action_loss_valid_dim: int | None = None  # 参与 loss 的有效 action 维数（配合上一项使用）

    # ---- Flow Matching 训练/推理 ----
    num_inference_steps: int = 10  # 推理去噪迭代步数（越多越慢、通常更稳）
    time_sampling_beta_alpha: float = 1.5  # 训练时时间步 t 的 Beta 分布 α
    time_sampling_beta_beta: float = 1.0  # 训练时时间步 t 的 Beta 分布 β
    time_sampling_scale: float = 0.999  # 时间步缩放上界
    time_sampling_offset: float = 0.001  # 时间步偏移，避免 t=0/1 边界
    min_period: float = 4e-3  # action 时间编码正弦周期下界
    max_period: float = 4.0  # action 时间编码正弦周期上界

    # ---- 注意力结构 ----
    attention_mask_mode: str = "default"  # default（按视角约束）或 causal（3D 因果消融，需 enable_3d_queries）

    # ---- 图像输入 ----
    image_resolution: tuple[int, int] = (224, 224)  # 送入模型的图像高宽
    image_delta_indices: list[int] = field(default_factory=lambda: [-15, 0, 15])  # 3 路相机帧的时间偏移（相对当前帧）
    empty_cameras: int = 0  # 占位空相机数量（多机体对齐用）

    normalization_mapping: dict[str, NormalizationMode] = field(  # 各模态归一化方式（TBot 默认恒等，归一化在 dataset 侧完成）
    ...
    # ---- 运行与加速 ----
    gradient_checkpointing: bool = False  # 梯度检查点，用算力换显存
    compile_model: bool = False  # 是否 torch.compile 加速
    compile_mode: str = "max-autotune"  # compile 模式
    device: str | None = None  # 运行设备，如 cuda / cpu

    # ---- 优化器（use_policy_training_preset=true 时生效）----
    optimizer_lr: float = 2.5e-5  # 学习率
    optimizer_betas: tuple[float, float] = (0.9, 0.95)  # AdamW β1、β2
    optimizer_eps: float = 1e-8  # AdamW ε
    optimizer_weight_decay: float = 0.01  # 权重衰减
    optimizer_grad_clip_norm: float = 1.0  # 梯度裁剪阈值

    # ---- 学习率调度 ----
    scheduler_warmup_steps: int = 1_000  # warmup 步数
    scheduler_decay_steps: int = 30_000  # 余弦衰减总步数
    scheduler_decay_lr: float = 2.5e-6  # 衰减后的最低学习率

    tokenizer_max_length: int = 48  # 语言指令 token 最大长度

    # ---- 微调策略：冻结 / LoRA ----
    freeze_vision_encoder: bool = False  # 冻结 Qwen3-VL 视觉编码器
    train_expert_only: bool = False  # 只训练 gen+act expert，冻结 und
    train_vlm_only: bool = False  # 只训练 und（VLM），冻结 gen+act
    lora_modules: tuple[str, ...] = ()  # 启用 LoRA 的 expert 子集：und / gen / act
    lora_unselected_mode: str = "full"  # 未选中的 expert：full（全量训）或 freeze（冻结）
    lora_targets: tuple[str, ...] = ("attn", "ffn")  # LoRA 注入位置：注意力 / FFN
    lora_rank: int = 16  # 默认 LoRA rank
    lora_alpha: float = 32.0  # 默认 LoRA alpha（缩放系数）
    lora_rank_und: int | None = None  # und expert 专用 rank，None 则用 lora_rank
    lora_alpha_und: float | None = None  # und expert 专用 alpha
    lora_rank_gen: int | None = None  # gen expert 专用 rank
    lora_alpha_gen: float | None = None  # gen expert 专用 alpha
    lora_rank_act: int | None = None  # act expert 专用 rank
    lora_alpha_act: float | None = None  # act expert 专用 alpha
    lora_dropout: float = 0.0  # LoRA dropout

    # ---- 2D 视觉规划辅助（loss_gen）----
    scale_factor: int = 8  # Cosmos latent 下采样倍率（与 middle visual token 数相关）
    lambda_gen: float = 0.01  # 图像重建辅助 loss 权重，0 可关闭
    cosmos_tokenizer_path_or_name: str = "nvidia/Cosmos-Tokenizer-CI8x8"  # Cosmos tokenizer 本地路径或 HF repo

    # ---- 3D 世界建模（loss_3d，DA3 teacher 蒸馏）----
    enable_3d_queries: bool = False  # 是否启用 future 3D query 分支
    num_3d_query_tokens: int = 1296  # middle 段 3D messenger query 总数（须能被 da3_num_views 整除）
    da3_alignment_mode: str = "query_decoder"  # 3D 对齐方式：query_decoder 或 upsample
    da3_query_resampler_layers: int = 1  # 兼容字段，当前固定为 1
    da3_query_resampler_ff_mult: int = 1  # 兼容字段，当前固定为 1
    query_layer_indices: tuple[int, ...] = (13, 19, 23, 27)  # 从 gen expert 哪些层抽取中间特征做 3D 对齐
    da3_variant: str = "auto"  # DA3 教师模型变体，auto 按路径自动推断
    da3_teacher_layers: tuple[int, ...] | None = None  # DA3 教师抽取层，None 则用模型默认
    da3_query_dim: int | None = None  # 3D query 投影维度，None 则与 DA3 特征维对齐
    da3_tokens_per_view: int = 1296  # 每个视角解码出的 DA3 对齐 token 数
    da3_num_views: int = 3  # 3D 对齐使用的视角数（与相机路数一致）
    lambda_3d: float = 0.05  # 3D 蒸馏 loss 权重，0 可关闭
    da3_model_path_or_name: str = "depth-anything/DA3-LARGE-1.1"  # DA3 教师权重路径或 HF repo
    da3_model_name: str | None = None  # da3_model_path_or_name 的别名（兼容旧配置）
    da3_code_root: str | None = None  # Depth-Anything-3 源码根目录（import 用）
    da3_teacher_process_res: int = 504  # DA3 教师推理时的图像短边分辨率
    da3_layer_weights: tuple[float, ...] = (1.0, 1.2, 1.4, 1.6)  # 各教师层 loss 的加权系数
    future_query_init_std: float = 0.02  # future_3d_queries 参数初始化标准差
    log_da3_teacher_timing: bool = False  # 是否打印 DA3 教师前向耗时（调试用）
