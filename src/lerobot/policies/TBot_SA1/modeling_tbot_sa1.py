...
# =============================================================================
# TBotSA1Model：核心神经网络（三段 embedding + 三 expert + 三个 loss 分支）
# =============================================================================
class TBotSA1Model(nn.Module):
    ...
    def forward(
        ...
        # 三段 embedding：prefix(语言+当前视觉) / middle(多视角+3D query) / suffix(state+带噪action)
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(
            pixel_values, image_grid_thw, lang_tokens, lang_masks
        )
...
# =============================================================================
# TBotSA1Policy：LeRobot 接口层（训练/推理入口，batch 解析与 loss 汇总）
# =============================================================================
class TBotSA1Policy(PreTrainedPolicy):
    """TBotSA1 策略：Qwen3-VL 感知 + expert action 头 + 可选 3D 对齐。"""
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        """训练入口：解析 batch → 调用 TBotSA1Model → 汇总总 loss。"""
