...
@DatasetConfig.register_subclass("bp_tbot_v2")
@dataclass
class BPTBotV2DatasetConfig(TBotSA1DatasetConfig):
    ...
    _canonical_type: ClassVar[str] = BP_TBOT_V2
    bp_num_chunks: int = 4
    ...
    data_transforms: TransformGroup = field(
        default_factory=lambda: TransformGroup(
            inputs=[
                ...
                BPRemapImageKeyTransformFn(),
                BPPadOrSampleChunksFn(num_chunks=BPTBotV2DatasetConfig.bp_num_chunks),
                BPResizeImagesWithPadFn(height=BPTBotV2DatasetConfig.height, width=BPTBotV2DatasetConfig.width),
                BPComposeFieldsTransform(),
                BPDeltaActionTransformFn(),
                BPNormalizeTransformFn(),
                BPPadStateAndActionTransformFn(
                    max_state_dim=BPTBotV2DatasetConfig.max_state_dim,
                    max_action_dim=BPTBotV2DatasetConfig.max_action_dim,
                ),
                # Current branch: mirror TBot transforms, but Qwen input is image-only.
                InjectMissingStateActionTransformFn(),
                ResizeImagesWithPadFn(height=BPTBotV2DatasetConfig.height, width=BPTBotV2DatasetConfig.width),
                RemapImageKeyTransformFn(),
                NormalizeTransformFn(),
                ComposeFieldsTransform(),
                PadStateAndActionTransformFn(
    ...
    def __post_init__(self):
        ...
        if str(original_action_mode).lower() == "obs":
            # Parent configs only validate abs/delta; BP_TBot_v2 treats obs as an input-only path.
...
@PreTrainedConfig.register_subclass("bp_tbot_v2")
@dataclass
class BPTBotV2Config(TBotSA1Config):
    ...
    # Temporary BP_TBot_v2 bootstrap: reuse the released TBot-SA1 base checkpoint and
