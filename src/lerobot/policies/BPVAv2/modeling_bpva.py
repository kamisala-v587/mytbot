...
from lerobot.policies.BPVAv2.bp_transformer_obs_encoder import BPObsEncoder, BPTransformerObsEncoder
...
try:
    from lerobot.policies.names import BPVAV2
except ImportError:
    BPVAV2 = "bpvav2"
...
class Future3DRefineBlock(nn.Module):
    ...
    def __init__(self, dim: int):
        ...
        # 逐位置的线性投影在此等价于 1x1 卷积，但能让 DDP 梯度桶保持更稳定的连续内存布局。
...
class Future3DPerceiverAttention(nn.Module):
    ...
    def _reshape_heads(self, x: torch.Tensor) -> torch.Tensor:
        """当前保留：_reshape_heads 用于 BPVAv2 训练、推理或兼容逻辑。"""
...
def infer_token_grid(num_tokens: int) -> tuple[int, int]:
    """将 token 数量分解为近似正方形的 2D 网格。

    大致功能为：3D query/teacher token 需要一个可解释的 HxW 布局用于日志和投影头构造；
    开发者通常不需要深入阅读。
    """
    if num_tokens <= 0:
...
def get_safe_dtype(target_dtype, device_type):
    """根据设备类型返回安全 dtype；CPU 上 bfloat16 回退到 float32。"""
...
def create_sinusoidal_pos_embedding(time: torch.Tensor, dimension: int, min_period: float, max_period: float, device="cpu") -> Tensor:
    """为 flow matching 时间步生成正弦-余弦位置编码。"""
    if dimension % 2 != 0:
...
def make_att_2d_masks(pad_masks, att_masks):
    """构造 2D attention mask。

    大致功能为：`pad_masks` 控制哪些 token 有效，`att_masks` 控制哪些位置开启新的注意力段。
    这是 openpi/TBot 原始逻辑，开发者通常只需要知道输出形状是 `(B, N, N)`。
    """
    if att_masks.ndim != 2:
...
def pad_vector(vector, new_dim):
    """用零把最后一维 pad 到 new_dim；如果已经足够长则原样返回。"""
...
def compute_layer_complete(layer_idx, inputs_embeds, attention_mask, position_ids, und_expert, gen_expert, act_expert):
    """三 expert 共享注意力的一层完整计算。

    大致功能为：prefix/middle/suffix 三段分别投影 Q/K/V，拼起来做一次联合 attention，
    再把结果切回三个 expert 各自过输出投影和 MLP。开发者通常不需要改这里。
    """
...
class QwenConfig:
    ...
    def __init__(self, head_dim, hidden_size, intermediate_size, num_attention_heads, num_hidden_layers, num_key_value_heads):
        """当前需要：保存 Qwen3-VL / Qwen3 text expert 的核心结构参数。"""
...
def get_qwen_config(variant: str) -> QwenConfig:
    ...
    大致功能为：把 `qwen3_vl_28l` / `qwen3_28l` 这种字符串转成 hidden size、层数、head 数。
    开发者通常只需要确认 variant 字符串和 checkpoint 规模一致。
    """
    num_hidden_layers = int(variant.split("_")[-1][:-1])  # pattern: qwen3_vl_28l or qwen3_28l
...
# =============================================================================
# Qwen3VLWithExpertModel：三 expert 联合 Transformer（每层共享注意力）
#   - und_expert: 语言 + Qwen3-VL 视觉理解 (prefix)
#   - gen_expert:  2D 视觉规划 / Cosmos 特征 (middle)
#   - act_expert:  state + 带噪 action (suffix)
# =============================================================================
class Qwen3VLWithExpertModel(
    nn.Module
):
    """TBotSA1 的 Qwen3-VL 主干 + 三个 expert 头（und / gen / act）。"""
...
# =============================================================================
# BPVAv2Model：核心神经网络（三段 embedding + 三 expert + 三个 loss 分支）
# =============================================================================
class BPVAv2Model(nn.Module):
    ...
    def __init__(self, config: BPVAv2Config):
        """当前需要：构建 BPVAv2 主干，并把 BPObsEncoder 纳入同一个 checkpoint。"""
        super().__init__()
        self.config = config
        ...
        # 主干：und(理解) + gen(生成) + act(动作) 三个 expert 共享注意力
        ...
        # Cosmos tokenizer：用于 2D 视觉规划辅助任务 (loss_gen)
        ...
        # BPObsEncoder 共享主干 Qwen visual，但保留每图全部视觉 tokens；每个 chunk
        # 经固定维度 query compressor 输出 Q 个 policy-D prefix tokens。
        prefix_dim = int(vlm_config.hidden_size)
        vision_config = self.qwen3_vl_with_expert.und_expert.config.vision_config
        visual_output_dim = int(vision_config.out_hidden_size)
        bp_chunk_encoder = BPTransformerObsEncoder(
            ...
            compressor_dim=config.bp_compressor_dim,
            state_action_hidden_dim=config.bp_state_action_hidden_dim,
            num_query_tokens=config.bp_num_query_tokens,
            num_layers=config.bp_compressor_num_layers,
            num_heads=config.bp_compressor_num_heads,
            ff_mult=config.bp_compressor_ff_mult,
            freeze_shared_visual=config.bp_freeze_shared_visual,
            use_type_embedding=config.bp_use_modality_type_embedding,
            use_camera_embedding=config.bp_use_camera_embedding,
            state_dim=config.max_state_dim,
            action_dim=config.max_action_dim,
            action_chunk_size=config.bp_action_chunk_size,
        )
        self.bp_obs_encoder = BPObsEncoder(
            chunk_encoder=bp_chunk_encoder,
            max_num_chunks=config.bp_num_chunks,
            use_chunk_position_embedding=config.bp_use_chunk_position_embedding,
        )
        ...
        # 3D 世界建模分支 (loss_3d)：future query + DA3 teacher 蒸馏
    ...
    def _prepare_attention_masks_4d(self, att_2d_masks):
        """将 2D 注意力掩码扩展为 Transformer 所需的 4D 格式。"""
    ...
    def _prepare_suffix_denoise_cache(
        ...
        """为去噪循环预先构建 suffix 的注意力掩码和 position_ids。

        推理时 suffix 布局固定：1 个 state token + chunk_size 个 action token。
        每步去噪只有 suffix embedding 在变，因此注意力掩码和 position_ids 可复用。
        """
    ...
    def embed_prefix(
        ...
        """按原始 TBot 方式编码当前观测 prefix。

        大致功能为：Qwen image placeholder 先拿文本 embedding 占位，再用 Qwen visual
        产出的图像 embedding 替换。开发者不需要完全阅读 Qwen 内部实现。
        """
        image_token_id = self.qwen3_vl_with_expert.und_expert.config.image_token_id

        # 1. 当前图像仍走原始 Qwen3-VL visual encoder。
        pixel_dim = pixel_values.shape[-1]
        image_embs, _ = self.qwen3_vl_with_expert.und_expert.visual(
            pixel_values.view(-1, pixel_dim),
            image_grid_thw.view(-1, 3),
        )

        # 2. lang_tokens 在 BPVAv2 中主要提供 image placeholder 的位置结构。
        ...
        token_ids_flat = lang_tokens.view(-1)
        embs_flat[token_ids_flat == image_token_id] = image_embs
        embs = embs_flat.view(batch_size, prefix_len, hidden_dim)

        # 3. prefix 内部全互通；suffix 仍按后续训练/推理逻辑接在后面。
        pad_masks = lang_masks.to(torch.bool)
        att_masks = torch.zeros_like(pad_masks, dtype=torch.bool, device=pad_masks.device)
        return embs, pad_masks, att_masks

    @dynamo.disable
    def embed_prefix_with_behavior_prompt(
        self,
        pixel_values: torch.Tensor,
        image_grid_thw: torch.Tensor,
        lang_tokens: torch.Tensor,
        ...
        """构造 BPVAv2 的最终 prefix。

        1. 当前观测按原始 TBot `embed_prefix` 编码，得到 `(B, L_current, 2048)`。
        2. behavior_prompt 交给 `BPObsEncoder`，K 个 chunk 各得到 Q 个 query tokens，
           flatten 后为 `(B, K*Q, 2048)`。
        3. 两段 token 直接 concat，形成 `(B, L_current + K*Q, 2048)`。
        4. 为新增 BP token 生成伪 token id，让后续 Qwen RoPE 位置计算可以复用原始接口。
        """
        # 1. 当前观测：完全复用原始 TBot 的 Qwen 图像占位 token 替换逻辑。
        current_embs, current_pad_masks, current_att_masks = self.embed_prefix(
            pixel_values,
            image_grid_thw,
            lang_tokens,
            lang_masks,
        ...
        # 2. BP 轨迹：固定相机槽位的完整视觉 tokens + state/action 压缩为 K*Q 个 query tokens。
        bp_encoded = self.bp_obs_encoder(
            behavior_prompt,
            visual=self.qwen3_vl_with_expert.und_expert.visual,
            return_dict=True,
        )
        bp_embs = bp_encoded.chunk_tokens
        if bp_embs.ndim == 2:
            bp_embs = bp_embs.unsqueeze(0)
        bp_embs = bp_embs.to(device=current_embs.device, dtype=current_embs.dtype)

        bp_pad_masks = bp_encoded.mask
        if bp_pad_masks is None:
            bp_pad_masks = torch.ones(bp_embs.shape[:2], dtype=torch.bool, device=current_embs.device)
        else:
            if bp_pad_masks.ndim == 1:
                bp_pad_masks = bp_pad_masks.unsqueeze(0)
            bp_pad_masks = bp_pad_masks.to(device=current_embs.device, dtype=torch.bool)
        if bp_pad_masks.shape != bp_embs.shape[:2]:
            raise ValueError(f"BP mask shape {tuple(bp_pad_masks.shape)} does not match BP embeddings {tuple(bp_embs.shape[:2])}")
        bp_att_masks = torch.zeros_like(bp_pad_masks, dtype=torch.bool, device=current_embs.device)

        # 3. 最终 prefix：current Qwen prefix + flatten 后的 BP query prefix。
        prefix_embs = torch.cat([current_embs, bp_embs], dim=1)
        prefix_pad_masks = torch.cat([current_pad_masks, bp_pad_masks], dim=1)
        prefix_att_masks = torch.cat([current_att_masks, bp_att_masks], dim=1)

        # 4. BP query 不是 Qwen 图像 token，因此 image_grid_thw 只保留当前观测。
        bp_token_ids = self._make_pseudo_token_ids(bp_pad_masks, lang_tokens)
        prefix_lang_tokens = torch.cat([lang_tokens, bp_token_ids], dim=1)
        prefix_image_grid_thw = image_grid_thw
        return prefix_embs, prefix_pad_masks, prefix_att_masks, prefix_lang_tokens, prefix_image_grid_thw

    @staticmethod
    def _make_pseudo_token_ids(pad_masks: torch.Tensor, token_reference: torch.Tensor) -> torch.Tensor:
        pseudo_token_id = 777
        return torch.full(
            pad_masks.shape,
            fill_value=pseudo_token_id,
            dtype=token_reference.dtype,
            device=token_reference.device,
        )
    ...
    def append_future_query_tokens(self, embs, pad_masks, att_masks):
        ...
        # cosmos 视觉 token 与 3D query 同属 middle 段，但拆成两个子块：
        # - visual/cosmos token 仅在视觉子块内双向互看
        # - 3D query token 在 query 子块内双向互看，且可读前面的 visual/cosmos 子块
        # 通过在第一个 query token 处开启新的注意力段来编码这一规则。
    ...
    def apply_view_aware_query_attention(
        ...
        """按视角限制 query 对 visual 的读取范围，同时保持 query 组内全互通。"""
        ...
        # 3D messenger 块保持双向，让各视角组可交换信息；
        # 同时限制每组只能从对应视角的 visual token 读取证据。
    ...
    def apply_causal_attention(
        ...
        """应用可选的 causal 消融注意力掩码。

        默认 TBotSA1 路径不会调用此方法。causal 模式下，
        middle visual、3D scene/query、action token 遵循如下块级关系：

            visual        -> current, visual, scene
            scene         -> current, scene, action tokens
            action tokens -> current, scene, state, action tokens

        state token 保持默认 suffix 行为：位于 action token 之前，
        action 可读 state，但 middle visual/scene 行不把 state 作为额外条件。
        在此消融设定下，state/action 的 suffix 行也不读 middle visual token。
        """
        if self.middle_query_token_count <= 0:
        ...
        # state 行：保持默认 suffix 位置，causal 消融下不读 middle visual
        for key_block in (prefix, scene, state):
            self._allow_attention_block(allowed, state, key_block)

        # action 行：可读 current、scene、state、action，但不读 middle visual
    ...
    def infer_middle_visual_token_count(self, images: torch.Tensor) -> int:
        """不实际跑 Cosmos，推断 middle 段会产出多少 visual token。"""
        _, num_views, timesteps = images.shape[:3]
        # embed_middle 会先把图像 resize 到 256，再经 Cosmos CI8x8 tokenizer，
        # 最后做 stride=scale_factor 的下采样投影。
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        """将 state、带噪 action、时间步编码为 suffix embedding（供 act expert 使用）。"""
        embs = []
        pad_masks = []
        att_masks = []
        ...
        # 设置注意力掩码：图像/语言/state 不能 attend 到 action token。
        # 注意这里必须使用真实 action token 数，而不是 config.chunk_size；验证/训练数据可能是 10，正式配置也可能是 50。
    ...
    def forward(
        self, images, img_masks, pixel_values, image_grid_thw, lang_tokens, lang_masks, state, actions, behavior_prompt, noise=None, time=None
    ) -> tuple[Tensor, dict[str, Tensor]]:
        """训练时的完整前向传播，返回 action loss 与辅助 loss。"""
        if noise is None:
        ...
        # Flow Matching：对 action 线性插值加噪
        time_expanded = time[:, None, None]
        x_t = time_expanded * noise + (1 - time_expanded) * actions
        u_t = noise - actions  # 训练目标：预测速度场 u_t

        # 三段 embedding：prefix(current视觉 + BP每chunk的Q个查询tokens) / middle / suffix。
        ...
        middle_embs, middle_pad_masks, middle_att_masks = self.embed_middle(
            images[:, :, :2], img_masks,  # 去掉未来观测帧，只保留前两帧
        )
        ...
        if pad_masks.shape != att_masks.shape:
            raise ValueError(
                "pad_masks and att_masks must have the same sequence length: "
                f"pad={tuple(pad_masks.shape)}, att={tuple(att_masks.shape)}, "
                f"prefix=({prefix_pad_masks.shape[1]}, {prefix_att_masks.shape[1]}), "
                f"middle=({middle_pad_masks.shape[1]}, {middle_att_masks.shape[1]}), "
                f"suffix=({suffix_pad_masks.shape[1]}, {suffix_att_masks.shape[1]})"
            )
        ...
        position_ids, rope_deltas = self.get_position_ids(prefix_lang_tokens, prefix_image_grid_thw, pad_masks)

        att_2d_masks_4d = self._prepare_attention_masks_4d(att_2d_masks)
        # 三 expert 联合前向；若启用 3D 则收集中间层输出供 loss_3d 使用
        ...
        # loss_gen：用 gen expert 输出重建未来帧的 Cosmos latent
    ...
    def sample_actions(
        ...
        if noise is None:
            # 按 action_in_proj 期望的填充维度采样噪声
            actions_shape = (
                bsize,
                self.config.chunk_size,
                self.config.max_action_dim,
            )  # 内部统一用 max_action_dim 处理
            noise = self.sample_noise(actions_shape, device)

        if self.uses_causal_attention():
            raise NotImplementedError(
                "BPVAv2 behavior-prompt inference is currently implemented only for default attention. "
    ...
    def sample_actions_causal(self, *args, **kwargs) -> tuple[Tensor, Tensor | None]:
        ...
        raise NotImplementedError(
            "BPVAv2 does not support causal inference yet; use attention_mask_mode='default'."
        )
...
# =============================================================================
# BPVAv2Policy：LeRobot 接口层（训练/推理入口，batch 解析与 loss 汇总）
# =============================================================================
class BPVAv2Policy(PreTrainedPolicy):
    """BPVAv2 策略：原始 TBot current prefix + 动态相机 BP chunk prefix。"""

    config_class = BPVAv2Config
    name = BPVAV2
    ...
    def __init__(
        ...
        """
        Args:
            config: 策略配置实例（TBotSA1Config）。
        """
        super().__init__(config)
        config.validate_features()
        self.config = config

        self.model = BPVAv2Model(config)
    ...
    def to(self, *args, **kwargs):
        ...
        # action 回归头保持 fp32，兼容现有 bf16 训练/推理路径
    ...
    def _get_loaded_pretrained_source_config(self):
        """Return any deserialized source config; compatibility is decided by its metadata."""
        return getattr(self, "_loaded_pretrained_source_config", None)

    def _should_load_bp_encoder(self) -> bool:
        source_config = self._get_loaded_pretrained_source_config()
        if getattr(source_config, "type", None) != "bpvav2":
            return False
        serialized_keys = getattr(source_config, "_serialized_keys", None)
        if serialized_keys is not None and "bp_encoder_version" not in serialized_keys:
            return False
        if getattr(source_config, "bp_encoder_version", None) != "query_compressor_v1":
            return False
        critical_fields = (
            "bp_camera_keys",
            "bp_num_query_tokens",
            ...
            "bp_action_chunk_size",
            "bp_num_chunks",
            "bp_compressor_ff_mult",
            "bp_use_modality_type_embedding",
            "bp_use_camera_embedding",
            "bp_use_chunk_position_embedding",
            "max_state_dim",
            "max_action_dim",
        )
        mismatches = [
            f"{name}: source={getattr(source_config, name, None)!r}, current={getattr(self.config, name, None)!r}"
            for name in critical_fields
            if getattr(source_config, name, None) != getattr(self.config, name, None)
        ]
        if mismatches:
            raise ValueError(
                "BPVAv2 query-compressor checkpoint is structurally incompatible: " + "; ".join(mismatches)
            )
        return True
    ...
    def _load_as_safetensor(
        ...
        load_bp_encoder = model._should_load_bp_encoder()
        skipped_legacy_bp_keys: list[str] = []
        for key, value in state_dict.items():
            if key.startswith("model.bp_obs_encoder.") and not load_bp_encoder:
                skipped_legacy_bp_keys.append(key)
                continue
        ...
        if unexpected_shape_keys:
            ...
            raise RuntimeError(
                "Unexpected shape mismatch while loading BPVAv2 pretrained weights: "
                f"{preview}{suffix}"
            )

        if skipped_legacy_bp_keys:
            logging.info(
                "Skipping %d incompatible/legacy BP encoder key(s); query_compressor_v1 is random-initialized.",
                len(skipped_legacy_bp_keys),
            )
        ...
        if skipped_shape_keys:
            ...
            suffix = f", ... (+{remaining} more)" if remaining > 0 else ""
            logging.info(
                "Skipping BPVAv2 shape-mismatched pretrained key(s); modules will be initialized from current config: %s%s",
                preview,
                suffix,
            )

        # Always collect incompatibilities first. Passing strict=True here would raise
        # before BPVAv2 can classify intentionally missing new/external modules.
        ...
        if strict and (missing_keys or unexpected_keys):
            ...
            raise RuntimeError(
                "Strict BPVAv2 checkpoint loading found non-expected incompatibilities: "
                + "; ".join(details)
            )
    ...
    def classify_model_loading_keys(
        ...
        """加载 TBot/BPVA/BPVAv2 checkpoint 时分类预期 missing/unexpected key。"""
        load_bp_encoder = self._should_load_bp_encoder()
        ...
        if not load_bp_encoder:
            ignored_missing_prefixes = ("model.bp_obs_encoder.", *ignored_missing_prefixes)
    ...
    def _preprocess_images(self, batch: dict[str, Tensor]) -> tuple[list[Tensor], list[Tensor]]:
        """从 batch 中取出 3 路相机图像及 mask，堆叠为模型输入格式。"""
        images = []
        img_masks = []
        ...
        images = torch.stack(images, dim=1)  # 形状: B, N_view, T, C, H, W
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        """根据观测选出单步 action（推理入口）。"""
        self.eval()

        # n_action_steps > 1 时用队列：一次预测多步，逐步弹出
        if len(self._action_queue) == 0:
            actions, _ = self.predict_action_chunk(batch)
            actions = actions[:, : self.config.n_action_steps]
            # 转置为 (n_action_steps, batch_size, action_dim)
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], decode_image=False) -> tuple[Tensor, Tensor | None]:
        ...
        if "behavior_prompt" not in batch:
            raise KeyError("BPVAv2 inference requires batch['behavior_prompt'] to build the BP prefix.")
        behavior_prompt = batch["behavior_prompt"]

        # 调用模型做多步去噪采样；BPVAv2 推理必须使用 behavior prompt prefix。
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        """训练入口：解析 batch → 调用 BPVAv2Model → 汇总总 loss。"""
        ...
        # 总 loss = action + λ_gen·图像重建 + λ_3d·3D 对齐
