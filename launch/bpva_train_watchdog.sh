...
#   nohup bash launch/bpva_train_watchdog.sh > logs/bpva_watchdog.nohup.log 2>&1 &
#
# 可选环境变量：
#   CUDA_DEVICE=0          使用的 GPU（默认 0）
...
INITIAL_CONFIG="${PROJECT_ROOT}/configs/bpva_train.jsonc"
OUTPUT_BASE="${PROJECT_ROOT}/outputs/BPVA/SFT-rand"
...
exec 9>"/tmp/my_tbot_bpva_train_watchdog.lock"
if ! flock -n 9; then
  echo "[$(date '+%F %T')] 已有 bpva_train_watchdog.sh 在运行，退出。"
  exit 1
fi
...
WATCHDOG_LOG="${LOG_DIR}/bpva_watchdog_$(date '+%F_%H-%M-%S').log"
CHILD_PID=""
CURRENT_RUN=""
ATTEMPT=0
STOP_REQUESTED=0
...
for value_name in CHECK_INTERVAL STALE_THRESHOLD STARTUP_GRACE RESTART_DELAY; do
  value="${!value_name}"
...
start_training() {
  ...
  local command=(accelerate launch --num_processes=1 -m lerobot.scripts.lerobot_train)
  ATTEMPT=$((ATTEMPT + 1))
  attempt_log="${LOG_DIR}/bpva_train_attempt_$(printf '%03d' "${ATTEMPT}")_$(date '+%F_%H-%M-%S').log"

  if [[ -n "${checkpoint}" ]]; then
    config_path="${checkpoint}/pretrained_model/train_config.json"
    command+=(--resume=true --config_path="${config_path}")
    CURRENT_RUN="$(dirname "$(dirname "${checkpoint}")")"
    log "第 ${ATTEMPT} 次启动：从 checkpoint 恢复：${checkpoint}"
  else
    command+=(--config_path="${INITIAL_CONFIG}")
    CURRENT_RUN=""
